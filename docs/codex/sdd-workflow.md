...
默认批量节奏：当用户要求继续后续 SDD 开发但没有指定数量时，默认按 8 个 SDD 为一批规划、执行和交付。每批开始前仍必须先完成“开始前收口上一轮”的检查；如果上一批尚未验收或不能提交，不得直接叠加下一批。
...
- 开始新一轮 SDD 开发前，必须先处理上一轮改动：优先提交已验收改动；如果暂不提交，必须在对话中明确说明原因、当前分支和剩余未提交范围。
...
创建或执行新的 SDD packet 前，先检查当前工作树和上一轮开发状态：

- 如果上一轮改动已验收且验证记录完整，先提交上一轮改动，再开始新的 SDD。
- 如果用户明确要求暂不提交，或工作树包含不属于上一轮的用户改动，不得强行提交；必须说明原因并保留改动。
- 如果上一轮尚未验收、验证失败或范围仍不清楚，先完成收尾或向用户确认，不得直接叠加新 SDD。
