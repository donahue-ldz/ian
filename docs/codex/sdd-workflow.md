...
- 涉及 Ian 桌面端可见能力、桌面窗口行为、Tauri API、拖动、位置、动画、气泡、鼠标交互或系统集成的 SDD，必须包含真实 Tauri 桌面端验收；浏览器预览验证只能作为补充，不能替代桌面端验收。
...
如果能力会在桌面端呈现或依赖 Tauri / 桌面窗口 API，`验证方式` 必须明确列出桌面端验收步骤，例如启动 Tauri 桌面壳、执行用户关键交互、观察窗口位置 / 动画 / 气泡 / 输入输出，并记录结果。浏览器预览、单元测试、类型检查和构建检查可以作为补充，但不得替代桌面端验收。
...
涉及 Ian 桌面端可见能力时，必须启动真实 Tauri 桌面端进行验收。验收至少覆盖用户关键路径、桌面窗口实际位置 / 交互命中区、动画或移动结果、气泡显示结果，以及桌面端控制台 / 运行日志中的明显错误。浏览器验证不能单独作为通过依据。
...
如果客观原因导致桌面端验收无法执行，必须在 `verification.md` 明确记录：
