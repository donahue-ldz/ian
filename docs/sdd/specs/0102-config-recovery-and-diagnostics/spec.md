# 0102 · Config Recovery And Diagnostics

## 问题 / 目标

让配置损坏、字段缺失或版本不兼容时可恢复，并能给开发者看到明确诊断，而不是静默失败或白屏。
...
- [ ] 损坏的 config.toml 不会导致应用启动失败。
- [ ] 缺失字段使用安全默认值并在下次保存时补齐。
- [ ] 日志记录错误类别，不记录 API key 或敏感值。
- [ ] 测试覆盖损坏 TOML、缺字段和未知字段。
