# 0096 · Desktop Safe Spawn And Recovery

## 问题 / 目标

强化桌面窗口安全出生点和越界恢复，避免 Ian 因显示器变化、缩放变化或坏配置跑到屏幕外。

## 当前产品阶段

P0 / v0.1 Baseline hardening。
...
- [ ] 保存位置越界时，启动后 Ian 出现在当前主显示器可见区域。
- [ ] 显示器尺寸变化后，下次启动不会跑到屏幕外。
- [ ] 恢复逻辑由 Rust Core 或 Tauri 桌面边界负责，React 不私自决定长期位置。
- [ ] 桌面端验收记录越界配置恢复结果。
