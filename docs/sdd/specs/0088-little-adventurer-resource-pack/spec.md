...
本轮重点不是继续复刻参考图，而是改成更适合桌面宠物的小体量像素风：块状轮廓、清晰表情、轻量配色、黄色发光探路道具、轻快冒险姿态，做成可长期扩展的原创 resource pack。
...
- 角色设定：像素风小冒险家 / 小动物混合体，短腿，小背包，软围巾，发光探路棒。
- 视觉气质：好奇、轻快、会探索、会撒娇，不做攻击性武器姿态。
- 视觉风格：4px 网格像素块，灰蓝主体、浅色脸部、青色围巾和黄色发光探路棒；避免大面积黑色剪影。
- 第一批实现必须覆盖现有语义动画：
  - `idle`
  - `walk`
  - `run`
...
- [ ] 角色视觉包含小背包、软围巾和发光探路棒，主体为轻量像素风格，不呈现攻击性武器姿态。
