# 0100 · Bubble Interruption And Tone Policy

## 问题 / 目标

建立气泡打断和语气策略，避免 Ian 在移动、拖动、设置或连续事件中乱弹气泡，保持短句、柔和、角色一致。
...
- [ ] 连续触发多个行为时，气泡不会快速闪烁或堆叠。
- [ ] 高优先级用户点击回复可以替换低优先级 idle 气泡。
- [ ] 所有默认短句保持中文优先、短句、非 AI assistant 口吻。
- [ ] 测试覆盖打断、冷却和替换规则。
